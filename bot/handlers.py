...
def extract_ca(text: str) -> str | None:
    """Pull a contract address out of ANYTHING: bare CA, a dexscreener /
    pump.fun / geckoterminal / photon link, or text around it."""
    t = text or ""
    m = re.search(r"0x[a-fA-F0-9]{40}", t)
    if m:
        return m.group(0)
    # base58-looking runs anywhere in the text (URLs included)
...
async def _dispatch_callback(update, context, query) -> None:
    ...
    if data.startswith("arc:"):
        from bot.arc_services import handle_callback

        await handle_callback(update, context, query, user)
        return
    ...
    if data == "nav:main":
        ...
        await safe_edit(
            query,
            "🧰 <b>More tools</b>\n\n"
            "Signals · Copytrade · Auto Snipe · Bridge\n"
            "Active Orders · Chains · Cashback · Referral · Settings",
            kb.tools_kb(),
        )
...
async def _on_text(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    # ARC services flow (tx submission state) — handled before trading states
    try:
        from bot.arc_services import maybe_handle_text

        if await maybe_handle_text(update, user, text):
            return
    except ImportError:
        pass
