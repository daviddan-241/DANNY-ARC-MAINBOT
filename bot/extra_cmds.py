...
log = logging.getLogger("solo-metro.extra")
...
def register_command_handlers(application: Application) -> None:
    ...
    def _gated(fn, name):
        async def wrapper(update, context):
            ...
            from bot.admintools import gate
            from bot.handlers import load_user

            try:
                _db.bump_stat(f"cmd:{name}")
            except Exception:
                pass
            user = load_user(update) if update.effective_user else None
            if user is None:
                return
            if await gate(update, user):
                return
    ...
    for names, fn in groups:
        application.add_handler(CommandHandler(names, _gated(fn, names[0])))

    # admin toolkit: bans, maintenance, announcements, stats
    try:
        from bot import admintools

        application.add_handler(CommandHandler(["ban"], admintools.cmd_ban))
        application.add_handler(CommandHandler(["unban"], admintools.cmd_unban))
        application.add_handler(CommandHandler(["announce", "broadcast"], admintools.cmd_announce))
        application.add_handler(CommandHandler(["maintenance"], admintools.cmd_maintenance))
        application.add_handler(CommandHandler(["adminstats"], admintools.cmd_adminstats))
    except Exception:
        log.exception("admin toolkit registration")

    # ARC admin mini app (admin-only launcher)
    try:
        ...
        async def cmd_admin_panel(update, context):
            ...
            if not url:
                await update.effective_message.reply_text(
                    "Admin panel needs RENDER_EXTERNAL_URL (Render sets it automatically)."
                )
                return
            await update.effective_message.reply_text(
                "🖥 <b>ARC Admin Panel</b>\nLive users, orders, trades — real data only.",
                parse_mode="HTML",
                reply_markup=InlineKeyboardMarkup([[
                    InlineKeyboardButton("🖥 Open Admin Panel",
        ...
        log.exception("admin mini app registration")

    # ARC services (user + admin order toolkit)
    try:
        from bot import arc_services

        application.add_handler(CommandHandler(["services", "service"], arc_services.cmd_services))
        application.add_handler(CommandHandler(["myorders", "myorders"], arc_services.cmd_myorders))
        application.add_handler(CommandHandler(["approve"], arc_services.cmd_approve))
        application.add_handler(CommandHandler(["reject"], arc_services.cmd_reject))
        application.add_handler(CommandHandler(["svcorders"], arc_services.cmd_orders_admin))
    except Exception:
        log.exception("arc services registration")
