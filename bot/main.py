...
# SECURITY: httpx logs every Telegram API URL at INFO — and the bot token is
# part of that URL (this leaked tokens into Render logs before). Silence it.
...
async def run_webhook(application: Application) -> None:
    ...
    try:
        from bot.adminweb import mount_admin

        mount_admin(http_app, application.bot)
    except Exception:
        log.exception("admin mini app mount")
