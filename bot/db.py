...
# tables moved aside by the last init_db (incompatible shapes from other apps)
_SCHEMA_RENAMED: list[str] = []


def _reconcile_incompatible(con) -> None:
    """If a table exists but is missing its FIRST (primary) column, it was
    created by a different app on this same database (e.g. an old 'users'
    with 'id' instead of 'user_id'). Rename it aside — data preserved — so
    the correct schema can be created. Without this, every query on that
    table fails with 'column does not exist'."""
    import re as _re
    import time as _time
    import logging as _logging

    log = _logging.getLogger("solo-metro.db")
    blocks = _re.findall(r"CREATE TABLE IF NOT EXISTS (\w+) \((.*?)\)\s*;", _SCHEMA_SQLITE, _re.S)
    suffix = f"pre_arc_{int(_time.time())}"
    for tname, body in blocks:
        first_col = body.strip().split("\n")[0].strip().split()[0]
        try:
            if ENGINE == "postgres":
                has_table = int(con.execute(
                    ...
                    (tname,)).fetchone()["n"])
                has_col = int(con.execute(
                    "SELECT COUNT(*) AS n FROM information_schema.columns "
                    "WHERE table_schema=current_schema() AND table_name=? AND column_name=?",
                    (tname, first_col)).fetchone()["n"]) if has_table else 1
            else:
                rows = con.execute(f"PRAGMA table_info({tname})").fetchall() or []
                has_table = 1 if rows else 0
                has_col = 1 if any(r[1] == first_col for r in rows) else 0
            if has_table and not has_col:
                con.execute(f'ALTER TABLE "{tname}" RENAME TO "{tname}_{suffix}"')
                _SCHEMA_RENAMED.append(f"{tname} → {tname}_{suffix}")
                log.warning(
                    "table %s had no column %s (created by another app) — renamed to %s_%s; "
                    "correct schema will be created", tname, first_col, tname, suffix)
        except Exception as exc:
            log.warning("reconcile %s: %s", tname, exc)


def init_db() -> None:
    if ENGINE == "postgres":
        with connect() as con:
            _reconcile_incompatible(con)
...
def get_service_order(order_id: int) -> Optional[dict]:
    with connect() as con:
        row = con.execute("SELECT * FROM services_orders WHERE id=?", (order_id,)).fetchone()
        return dict(row) if row else None


def list_service_orders(user_id: int) -> list:
    with connect() as con:
        rows = con.execute(
            "SELECT * FROM services_orders WHERE user_id=? ORDER BY id DESC LIMIT 20", (user_id,)
        ).fetchall()
        return [dict(r) for r in rows]


def all_service_orders(limit: int = 20) -> list:
    with connect() as con:
        rows = con.execute(
            "SELECT * FROM services_orders ORDER BY id DESC LIMIT ?", (limit,)
        ).fetchall()
        return [dict(r) for r in rows]


def set_service_order_status(order_id: int, status: str, tx: str = "") -> None:
    with connect() as con:
        if tx:
            con.execute(
                "UPDATE services_orders SET status=?, tx=? WHERE id=?", (status, tx, order_id)
            )
        else:
            con.execute("UPDATE services_orders SET status=? WHERE id=?", (status, order_id))
...
# ----- admin: bans, activity, usage stats -----
def ban_user(user_id: int, banned: bool = True) -> None:
    with connect() as con:
...
def touch_active(user_id: int) -> None:
    with connect() as con:
        con.execute("UPDATE users SET last_active=? WHERE user_id=?", (time.time(), user_id))
...
def stat_counts(limit: int = 15) -> list:
    with connect() as con:
        rows = con.execute(
            "SELECT key, count FROM usage_stats ORDER BY count DESC LIMIT ?", (limit,)
        ).fetchall()
        return [dict(r) for r in rows]


def stat_total(prefix: str = "") -> int:
    with connect() as con:
        row = con.execute(
            "SELECT COALESCE(SUM(count),0) AS s FROM usage_stats WHERE key LIKE ?",
            (prefix + "%",),
        ).fetchone()
        return int(row["s"])


def count_active_since(ts: float) -> int:
    with connect() as con:
        row = con.execute(
            "SELECT COUNT(*) AS n FROM users WHERE last_active >= ? AND banned=0", (ts,)
        ).fetchone()
        return int(row["n"])


def list_users_admin(search: str = "", limit: int = 100) -> list:
    with connect() as con:
        if search:
            like = f"%{search}%"
            rows = con.execute(
                "SELECT user_id, username, first_name, verified, premium, banned, "
                "created_at, last_active FROM users "
            ...
            rows = con.execute(
                "SELECT user_id, username, first_name, verified, premium, banned, "
                "created_at, last_active FROM users ORDER BY user_id DESC LIMIT ?",
                (limit,),
            ).fetchall()
        return [dict(r) for r in rows]
