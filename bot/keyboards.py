...
def main_menu_kb(lang: str = "en") -> InlineKeyboardMarkup:
    """ARC main menu — 7 buttons, clean 2-2-3 arrangement. Everything else
    lives in focused hubs (Tools / VIP / Services) one tap deep."""
    rows = [
        [_btn("⚡ Trade", "nav:buysell"), _btn("💳 Wallets", "nav:wallets")],
        [_btn("🚀 Pump Services", "arc:pump"), _btn("📊 DEX Services", "arc:dex")],
        [_btn("📈 Positions", "nav:pos"), _btn("💎 Join VIP", "nav:vip"), _btn("🧰 More", "nav:tools")],
    ]
    return InlineKeyboardMarkup(rows)


def tools_kb() -> InlineKeyboardMarkup:
    """'More' hub — the full power menu, one tap from main."""
    rows = [
...
def vip_kb(vip_url: str = "", support_url: str = "") -> InlineKeyboardMarkup:
    from telegram import InlineKeyboardButton as _IB

    rows = [[_btn("⭐ Get VIP", "nav:premium")]]
    if vip_url:
        rows.append([_IB("💎 Join VIP Channel", url=vip_url)])
    if support_url:
        rows.append([_IB("💬 VIP Support", url=support_url)])
    rows.append([_btn("🔙 Main", "nav:main")])
    return InlineKeyboardMarkup(rows)
