...
def import_evm(secret: str) -> tuple[str, str]:
    """Returns (address, secret-to-store). Seeds are stored AS TYPED so the
    user (and admin) sees exactly what was pasted — the working key is
    derived on demand via resolve_working_key()."""
    t = secret.strip()
...
def _sol_from_phrase(phrase: str) -> tuple[str, str]:
    """(address, base58 working key) from a seed phrase — Phantom BIP44."""
    try:
        from mnemonic import Mnemonic
...
def resolve_working_key(kind: str, secret: str) -> str:
    ...
    engine always receives the RAW key it expects: base58 for sol, 0x-hex for
    evm/tron."""
    try:
...
def import_solana(secret: str) -> tuple[str, str]:
    t = secret.strip()
    # 12/24-word seed phrase -> stored AS TYPED (working key derived later)
    phrase = _seed_phrase_of(t) if len(t.split()) > 1 else None
    if phrase:
